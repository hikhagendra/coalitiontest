...
function ct_custom_scripts() {
	...
	wp_enqueue_style( 'ct-custom-main', get_template_directory_uri() . '/css/main.css' );
	wp_enqueue_style( 'barlow', 'https://fonts.googleapis.com/css2?family=Barlow:wght@300;400;500;600;700&display=swap' );
 ...
 */
function ct_custom_load_admin_scripts( $hook ) {
    // echo $hook;
    if('toplevel_page_theme_settings' == $hook) {

        wp_enqueue_media();

        wp_register_script( 'ct_custom-admin-script', get_template_directory_uri() . '/js/coalition.admin.js', array('jquery'), '1.0.0', true );
        wp_enqueue_script( 'ct_custom-admin-script' );
    }
}

add_action( 'admin_enqueue_scripts', 'ct_custom_load_admin_scripts' );
